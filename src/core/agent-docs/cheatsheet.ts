...
export function render(program: Command): string {
  ...
  return [
    ...
    '3. Work: `tabbrew tabs list --json` lists every tab, window and group; `tabbrew windows list --json` summarises each window; `tabbrew groups list --json` lists every tab group; `tabbrew groups collapse <group...>` / `uncollapse` fold or expand groups by id; `tabbrew tabs move <tab...> --after <tab>` / `--before <tab>` puts tabs next to another tab, in its window; `tabbrew tabs group <tab...> --to <group>` / `--window <window>` joins or creates a tab group (add `--title`/`--color` to set them); `tabbrew tabs discard <tab...>` unloads tabs from memory without closing them.',
