...
export function render(program: Command): string {
  ...
  return [
    ...
    '3. Open the TabBrew panel in Chrome (toolbar icon), then work: `tabbrew tabs list --json` lists every tab, window and group; `tabbrew windows list --json` summarises each window; `tabbrew groups list --json` lists every tab group.',
